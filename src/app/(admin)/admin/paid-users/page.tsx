...
export default function PaidUsersPage() {
    ...
    const fetchData = async (f: FilterValues) => {
        setIsLoading(true);
        try {
            const res = await getAdminRecords(page, 10, f.search || "", "paid", f.startDate, f.endDate);
            if (res.ok && res.data) {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-bold">Paid Users</h1>
                    <p className="text-slate-500 mt-1">Manage users who have completed payment.</p>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 bg-green-500/10 text-green-600 rounded-lg h-10">
