# Seed Data Script Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a single `scripts/seed.mjs` that populates the BRPL admin panel with realistic backdated dummy data for Users, Coupons, Referrals, CouponUsage, and Payments.

**Architecture:** Single Mongoose script that connects to MongoDB, creates properly interlinked documents across all 5 collections with backdated timestamps, and disconnects. Follows existing `scripts/create-test-user.mjs` pattern.

**Tech Stack:** Node.js, Mongoose, dotenv

## Global Constraints

- Models are loaded via `mongoose.model(name)` — not imported as ES modules (the script is `.mjs`)
- Script uses `dotenv` to load `.env.local` (already in devDependencies)
- Phone numbers must be unique 10-digit strings: seed uses `9000000001` through `9000000012`
- Coupon codes are uppercase and unique: codes designed not to collide
- Referral codes are uppercase and unique
- All payment IDs use `pay_seed_*` prefix for clear identification
- `--force` flag clears seed data before re-inserting

---
### Task 1: Write and verify `scripts/seed.mjs`

**Files:**
- Create: `scripts/seed.mjs`

**Interfaces:**
- None (standalone script, no exports)

This is a single-task plan because the deliverable is one self-contained file. All the design detail is in the spec — this task writes the full script.

- [ ] **Step 1: Scaffold the script — imports, env, connect, CLI args**
...
- [ ] **Step 2: Register models** (load schemas by calling `mongoose.model` with name + schema so the script can use them)
...
- [ ] **Step 3: Implement `daysAgo(n)` helper and `--force` clear logic**
...
- [ ] **Step 4: Create coupons** (5 manual, 2 referral-linked later)
...
- [ ] **Step 5: Create referral coupons + referrals**
...
- [ ] **Step 6: Create users with coupon/payment relationships**
...
- [ ] **Step 7: Create CouponUsage records**
...
- [ ] **Step 8: Create Payment records for paid users**
...
- [ ] **Step 9: Wire everything together — main() with summary**
...
- [ ] **Step 10: Run the script and verify output**

Run:
```bash
cd "/Users/anurag/Desktop/untitled folder/brpl-frontend copy 2"
node scripts/seed.mjs
```

Expected: Script connects, creates all seed data, prints summary table with counts:
- Coupons: 7
- Referrals: 2
- Users: 12
- Coupon Usage: 7
...
- [ ] **Step 11: Run with --force to verify idempotency**
...
Expected: Clears existing seed data, re-inserts, same summary counts. Run a second time without `--force` — should say "Seed data already exists. Use --force to re-seed."

- [ ] **Step 12: Commit**

```bash
git add scripts/seed.mjs docs/superpowers/specs/2026-06-29-seed-data-design.md docs/superpowers/plans/2026-06-29-seed-data.md
git commit -m "feat: add seed data script for admin panel

Populates Users, Coupons, Referrals, CouponUsage, and Payments
with realistic backdated dummy data for development/demo use.

Run: node scripts/seed.mjs (or with --force to re-seed)
