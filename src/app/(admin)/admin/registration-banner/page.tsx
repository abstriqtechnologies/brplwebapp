...
export default function RegistrationBannerPage() {
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await getRegistrationSection("banner");
    ...
    const save = async () => {
        setSaving(true);
        try {
            const res = await updateRegistrationSection("banner", data);
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold">Form Banner & Quote</h1>
                    <p className="text-slate-500 mt-1">Banner + inspirational quote shown alongside the registration form.</p>
                </div>
            ...
            <Card>
                <CardHeader><CardTitle>Banner & Quote</CardTitle></CardHeader>
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="md:col-span-2"><Label>CTA Link</Label><Input value={data.ctaLink || ""} onChange={(e) => setData({ ...data, ctaLink: e.target.value })} /></div>
                </CardContent>
            </Card>
        </div>
    );
