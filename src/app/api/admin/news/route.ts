/**
 * `/api/admin/news` — admin CRUD list/create for news articles.
 *
 *   GET  /api/admin/news?search=&page=&pageSize=
 *   POST /api/admin/news
 *
 * Auth: superadmin.
 */
...
function serializeNews(doc: NewsDoc) {
    return {
        id: String(doc._id),
        title: doc.title,
        slug: doc.slug,
...
export const GET = withRequest(
    withAdmin({
        ...
        return ok({
            news: (news as unknown as NewsDoc[]).map(serializeNews),
            total,
            page,
            pageSize,
        });
...
export const POST = withRequest(
    withAdmin({
        ...
        try {
            ...
            return ok({ news: serializeNews(article.toObject() as NewsDoc) });
        } catch (err) {
            const e = err as { code?: number };
            if (e?.code === 11000) {
                throw new ConflictError(`A news article with slug "${data.slug}" already exists`);
            }
            throw err;
        }
    }),
